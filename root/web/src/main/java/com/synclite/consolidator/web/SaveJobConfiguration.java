	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			for (Integer dstIndex = 1 ; dstIndex <= numDestinations ; ++dstIndex) {
				...
				builder.append("metadata-store-" + dstIndex + " = ").append(session.getAttribute("metadata-store-" + dstIndex) != null ? session.getAttribute("metadata-store-" + dstIndex).toString() : "DESTINATION").append("\n");
