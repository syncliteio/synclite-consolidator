...
public class SyncLiteAppLock {
	...
	public final void tryLock(Path workDir) throws SyncLiteException {		
		this.lockFile = workDir.resolve("synclite_consolidator.lock");
		try {
			...
			if (this.lock == null) {
				...
				throw new SyncLiteException("Failed to lock synclite consolidator lock " + this.lockFile
						+ ". Another consolidator job appears to be running for work-dir : " + workDir
			...
			String existingOwner = readLockOwnerInfo();
			throw new SyncLiteException("Failed to lock synclite consolidator lock " + this.lockFile
					+ ". Another consolidator job is already running in this JVM for work-dir : " + workDir
					+ ". Existing lock owner details: " + existingOwner);
		} catch (Exception e) {
			release();
			throw new SyncLiteException("Failed to lock synclite consolidator lock " + this.lockFile
					+ ". Please stop any other consolidator job running for the specified work-dir : " + workDir
